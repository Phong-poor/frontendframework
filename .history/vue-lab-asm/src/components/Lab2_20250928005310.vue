...
const handleXuLyDiemTrungBinh = () => {
    if(diem.value == ""){
        message.value = "Chưa có thông tin"
    }
    else if(diem.value > 0 && diem.value < 5){
        message.value = "Xếp loại yếu"
    }
    else if(diem.value >= 5 && diem.value < 6.5){
        message.value = "Xếp loại trung bình"
    }
    else if(diem.value >= 6.5 && diem.value < 8){
        message.value = "Xếp loại khá"
    }
    else if(diem.value >= 8 && diem.value < 9){
        message.value = "Xếp loại giỏi"
    }
    else if(diem.value >= 9 && diem.value <= 10){
        message.value = "Xếp loại xuất xắc"
    }
    else if(diem.value > 10){
        message.value = "Điểm trung bình phải nhỏ hơn 10"
    }
}
</script>
<template>
