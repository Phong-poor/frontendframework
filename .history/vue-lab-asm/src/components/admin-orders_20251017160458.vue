<script setup>
    import { ref, onMounted } from 'vue'

    // Danh sách đơn hàng (giả lập — sau này bạn thay bằng API hoặc localStorage)
    const orders = ref([])

    onMounted(() => {
    // Giả lập dữ liệu — sau này thay bằng fetch từ server
    orders.value = [
        { id: 'DH001', customerName: 'Nguyễn Văn A', phone: '0900000000', total: 2500000, status: 'Chờ xác nhận' },
        { id: 'DH002', customerName: 'Trần Thị B', phone: '0911222333', total: 1800000, status: 'Đang giao' },
    ]
    })

    // Hàm cập nhật trạng thái
    function updateStatus(id, newStatus) {
    const order = orders.value.find(o => o.id === id)
    if (order) {
        order.status = newStatus
    }
    }

    // Hàm định dạng tiền tệ
    function formatCurrency(amount) {
    return amount.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })
    }
</script>

<template>
    <div class="page-wrapper">
        <header class="py-4 bg-white border-bottom mb-4">
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-4">
                    <h1 class="h4 mb-0">MyShop</h1>
                    </div>
                    <div class="col-4 text-center">
                    <router-link to="/" class="btn btn-primary">Trở lại trang chủ</router-link>
                    </div>
                    <div class="col-4 text-end">
                    <router-link to="/thongtincanhan" class="btn btn-success">Tài khoản</router-link>
                    <button @click="handleDangXuat" class="btn btn-outline-danger">Đăng xuất</button>
                    </div>
                </div>
            </div>
        </header>
        <main class="container pb-5">

        </main>
        <footer class="py-4 bg-dark text-white">
            <div class="container d-flex flex-wrap justify-content-between align-items-center gap-3">
                <span>© <span id="year">2025</span> MyShop</span>
                <a class="btn btn-outline-light btn-sm" href="#top">Back to top</a>
            </div>
        </footer>
    </div>
</template>

<style scoped>
    .page-wrapper {
        display: flex;
        flex-direction: column;
        min-height: 100vh;
    }
    main {
        flex: 1;
    }
    body {
        background-color: #f8f9fa;
        margin: 0;
    }
    .btn-outline-danger{
        margin-left: 10px;
    }
</style>