<script setup>
    ...
    if (response.status == 200) {
        ...
        // Giả sử người dùng hiện tại được lưu trong localStorage
        const currentUser = JSON.parse(localStorage.getItem('currentUser'));
        if (currentUser) {
        const foundUser = users.value.find(u => u.id === currentUser.id);
        if (foundUser) Object.assign(user, foundUser);
        }
    }
    };

    // Cập nhật thông tin người dùng (KHÔNG có thêm mới)
