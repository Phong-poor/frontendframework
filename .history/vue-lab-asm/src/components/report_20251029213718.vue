...
const calculateSalesData = (period = 'all') => {
    ...
    orders.value.forEach(order => {
        let rawDate = order.created_at || order.date || order.createdAt || order.ngaydat

        // nếu là timestamp (số) → convert thẳng
        if (typeof rawDate === 'number') {
        rawDate = new Date(rawDate)
        } else if (typeof rawDate === 'string') {
        // nếu là kiểu dd/mm/yyyy → chuyển sang yyyy-mm-dd
        if (rawDate.includes('/')) {
        ...
        if (period !== 'all' && orderDate < startDate) return
