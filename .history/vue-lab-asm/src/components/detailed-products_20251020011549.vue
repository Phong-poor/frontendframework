    ...
    onMounted(async () => {
        ...
        if (products.value.category) {
            ...
            relatedProducts.value = relatedResponse.data
                .filter(item => item.id != products.value.id)
                .map(item => ({
                    ...item,
            ...
            <div v-if="relatedProducts.length > 0" class="mt-5">
                ...
                <div class="row g-4">
                    <div v-for="item in relatedProducts" :key="item.id" class="col-6 col-md-3">
                        <div class="card h-100 shadow-sm related-card d-flex flex-column">
                            <img :src="item.image" :alt="item.name" class="card-img-top related-image" />
                                ...
                                <!-- Comment sản phẩm -->
                                <div class="related-comments mb-2">

                                </div>                                    <div v-if="item.comments?.length > 0">
                                        <div v-for="(cmt, index) in item.comments.slice(0,2)" :key="index" class="comment-item">
                                        <strong>{{ cmt.username }}:</strong> {{ cmt.comment }}
                                        </div>
                                        <div v-if="item.comments.length > 2" class="text-muted small">
                                        +{{ item.comments.length - 2 }} đánh giá khác
                                        </div>
                                    </div>
                                    <div v-else class="text-muted small">Chưa có đánh giá</div>
