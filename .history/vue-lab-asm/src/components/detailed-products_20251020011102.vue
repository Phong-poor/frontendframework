    ...
    <div class="card-body d-flex flex-column justify-content-between flex-grow-1">
      ...
      <div class="related-comments mb-2">
        <div v-if="item.comments.length">
          <div v-for="(cmt, index) in item.comments.slice(0,2)" :key="cmt.id" class="comment-item">
            <strong>{{ cmt.username }}:</strong> {{ cmt.comment }}
          </div>
          <div v-if="item.comments.length > 2" class="text-muted small">+{{ item.comments.length - 2 }} đánh giá khác</div>
        </div>
