<script setup>
    ...
    const handleReorder = async (order) => {
        try {
            for (const item of order.items) {
            // Kiểm tra sản phẩm trong giỏ hàng hiện tại (API)
            const cartRes = await axios.get(`http://localhost:3000/cart?id=${item.id}`);
            
            if (cartRes.data.length > 0) {
                // Nếu đã có sản phẩm thì cộng thêm số lượng
                const existing = cartRes.data[0];
            ...
            alert(`Đơn hàng #${order.id} đã được thêm lại vào giỏ hàng 🛒`);
            // 👉 Chuyển hướng sang trang giỏ hàng (nếu bạn có route này)
            router.push('/giohang');
        } catch (error) {
            console.error('Lỗi khi mua lại đơn hàng:', error);
            alert('Đã xảy ra lỗi khi mua lại đơn hàng!');
        }
        };
