<script setup>
    ...
    if (product.value.category) {
        // gọi thêm các sản phẩm cùng danh mục
        const relatedResponse = await axios.get(`http://localhost:3000/products?category=${product.value.category}`)
        // loại bỏ sản phẩm hiện tại ra khỏi danh sách
        relatedProducts.value = relatedResponse.data.filter(item => item.id != product.value.id)
    }
    })
